...
def mysqlconnect():
    ...
    toCSV = []
    try:
        cursor.execute("SELECT * FROM [DL_Gonukkad].[dbo].[MerchantCallBack]")
...
new_directory = os.path.join(os.getcwd(), 'gon_call_back_responses')
...
yesterday_filename = 'MerchantCallBack_'+yesterday.strftime("%Y-%m-%d")+".csv"
rows = mysqlconnect()
csv_file_path = new_directory+'/'+yesterday_filename
fieldnames = list(rows[0].keys())
# fieldnames = ['PK_GMBMerchantLockRequestManagerCallID', 'MerchantID', 'RequestManagerNo','Type','CreatedDateTime','PK_GMBId','BusinessName','MobileNo','IVRSNumber']
...
def upload_to_bucket(blob_name, path_to_file, bucket_name):
    ...
    folder_name = 'live/MerchantCallBack/'
...
upload_to_bucket('live/MerchantCallBack/'+yesterday_filename,csv_file_path,'bucket_name')
...
source_table = 'gonukkad.temp_LIVE_Merchant_Call_Back'
destination_table = 'gonukkad.LIVE_Merchant_Call_Back'
