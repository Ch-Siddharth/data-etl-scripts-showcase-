...
def mysqlconnect():
    ...
    toCSV = []
    try:
        cursor.execute("SELECT * FROM [DL_Gonukkad].[dbo].[GMBMerchantReviewItem]")
...
new_directory = os.path.join(os.getcwd(), 'gon_merchant_review_items_responses')
...
yesterday_filename ='GMBMerchantReviewItem_'+yesterday.strftime("%Y-%m-%d")+".csv"
rows = mysqlconnect()
csv_file_path = new_directory+'/'+yesterday_filename
fieldnames = list(rows[0].keys())
# fieldnames = ['Pk_ReviewItemId', 'GBMID', 'GoogleAccountId', 'GoogleLocationId', 'ReviewId', 'IsGood', 'ItemName', 'CreatedBy', 'CreatedDate', 'UpdateBy', 'UpdateDate', 'Fk_CompanyId', 'IsActive', 'IsDeleted']
...
def upload_to_bucket(blob_name, path_to_file, bucket_name):
    ...
    #renaming files inside path
    folder_name = 'live/GMBMerchantReviewItem/'
...
upload_to_bucket('live/GMBMerchantReviewItem/'+yesterday_filename,csv_file_path,'bucket_name')
...
source_table = 'gonukkad.temp_Live_Merchant_Review_Item'
destination_table = 'gonukkad.Live_Merchant_Review_Item'
